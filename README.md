...
| Option                | Default          | Details                                                                                                                                                                               | Option `func`                          | Can set on container tag        |
| :-------------------- | :--------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------------- | :------------------------------ |
| `Tag`                 | `"label"`        | `Tag` is the name of the tag to lookup. This is especially handy if you have multiple sources of labels                                                                               | `OptTag(t string)`                     | No                              |
| `Seperator`           | `","`            | Seperates the tag attributes. Configurable incase you have a tag that contains commas.                                                                                                | `OptSeperator(v string)`               | No                              |
| `ContainerToken`      | `"*"`            | Used to indicate the field containing the `map[string]string` if you do not want to implement mutator/accessor interfaces. This can also be used to set some labels (see last column) | `OptSeperator(v string)`               | No                              |
| `AssignmentStr`       | `":"`            | Used to assign values. This is in the event that a default value needs to contain `":"`                                                                                               | `OptAssignmentStr(v string)`           | No                              |
| `RequireAllFields`    | `false`          | If `true`, all fields are required by default. An `ParsingError` will be returned from `Unmarshal` containing a `FieldError` for each field that was not present in the labels        | `OptRequireAllFields()`                | Yes `RequireToken`              |
| `KeepLabels`          | `true`           | Indicates whether or not labels that have been assigned to values are kept in the labels `map[string]string` when unmarshaling.                                                       | `OptKeepLabels()` `OptDiscardLabels()` | Yes `KeepToken`, `DiscardToken` |
| `IgnoreCase`          | `true`           | If `true`, label keys are matched regardless of case. Setting this to `false` makes all keys case sensitive. This can be overriden at the field level.                                | `OptCaseSensitive()`                   | Yes `CaseSensitiveToken`        |
| `TimeFormat`          | `""`             | Default format / layout to use when formatting `time.Time`. Field level formats can be provided with either `format` (configurable) or `timeformat` (configurable)                    | `OptTimeFormat(v string)`              | Yes `TimeFormatToken`           |
| `FloatFormat`         | `'f'`            | Default format to use when formatting `float` values.                                                                                                                                 | `OptFloatFormat(f byte)`               | Yes `FloatFormatToken`          |
| `FormatToken`         | `"format"`       | Token to set the field-level formatting for `time` and `float`.                                                                                                                       | `OptFormatToken(v string)`             | No                              |
| `FloatFormatToken`    | `"floatformat"`  | Token used to set `FloatFormat`. This is really only relevant on container fields as `FormatToken` can be used at the field level.                                                    | `OptFloatFormatToken(v string)`        | No                              |
| `TimeFormatToken`     | `"timeformat"`   | Token used to set `TimeFormat`. This is really only relevant on container fields as `FormatToken` can be used at the field level.                                                     | `OptTimeFormatToken(v string)`         | No                              |
| `RequiredToken`       | `"required"`     | Token to mark a field as required. If applied to a container, all fields are required unless `NotRequiredToken` is present.                                                           | `OptRequiredToken(v string)`           | No                              |
| `NotRequiredToken`    | `"notrequired"`  | Token to mark a field as not required. Only relevant if `RequireAllFields` has been set to `true`.                                                                                    | `OptRequiredToken(v string)`           | No                              |
| `DefaultToken`        | `"default"`      | Token to provide a default value if one is not set. Stylistic.                                                                                                                        | `OptDefaultToken(v string)`            | No                              |
| `CaseSensitiveToken`  | `"casesensitive` | Token used to set `IgnoreCase` to `false`                                                                                                                                             | `OptCaseSensitiveToken(v string)`      | No                              |
| `IgnoreCaseToken`     | `"ignorecase"`   | Token used to determine whether or not to ignore case of the field's (or all fields if on container) key                                                                              | `OptIgnoreCaseToken(v string)`         | No                              |
| `KeepToken`           | `"keep"`         | Token used to set `KeepLabels` to `true`                                                                                                                                              | `OptKeepToken(v string)`               | No                              |
| `DiscardToken`        | `"discard"`      | Token used to set `KeepLabels` to `false`                                                                                                                                             | `OptDiscardToken(v string)`            | No                              |
| `CaseSensitiveTokens` | `false`          | Determines whether or not tokens, such as `required` and `ignorecase`, are case sensitive. This does not affect whether a label's key is case sensitive.                              | `OptCaseSensitiveTokens(v bool)`       | No                              |
