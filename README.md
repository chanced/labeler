...
| `RequireAllFields`    | `false`          | If `true`, all fields are required by default. An `ParsingError` will be returned from `Unmarshal` containing a `FieldError` for each field that was not present in the labels        | `OptRequireAllFields()`                | Yes                      |
| `KeepLabels`          | `true`           | Indicates whether or not labels that have been assigned to values are kept in the labels `map[string]string` when unmarshaling.                                                       | `OptKeepLabels()` `OptDiscardLabels()` | Yes                      |
| `IgnoreCase`          | `true`           | If `true`, label keys are matched regardless of case. Setting this to `false` makes all keys case sensitive. This can be overriden at the field level.                                | `OptCaseSensitive()`                   | Yes                      |
| `TimeFormat`          | `""`             | Default format / layout to use when formatting `time.Time`. Field level formats can be provided with either `format` (configurable) or `timeformat` (configurable)                    | `OptTimeFormat(v string)`              | Yes                      |
| `FloatFormat`         | `'f'`            | Default format to use when formatting `float` values.                                                                                                                                 | `OptFloatFormat(f byte)`               | Yes                      |
