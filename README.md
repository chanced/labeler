...
| `ContainerField`      |   `""`    | `ContainerField` is not required if `input` implements the appropriate `interface` to retrieve and set labels. If `ContainerField` is set, the `type` of `input` while Unmarshal is not checked and `ContainerField` is instead. | `OptContainerField(s string)`          |
