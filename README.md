...
| `ContainerField`      |       `""`        | `ContainerField` is not required if `input` implements the appropriate `interface`s. It basically must have `GetLabels` and `SetLabels` methods but see above for more detail. If `ContainerField` is set, the `type` of `input` while Unmarshaling is not checked and `ContainerField` is instead. This can be set with a field tagged using the `ContainerToken` as the value. | `OptContainerField(s string)`          |
