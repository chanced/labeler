...
// accepts map[string]string and handles assignment of those values. An error
// will be returned from Marshal if v does not contain a tag with `label:"*"`,
...
func (field labelField) setValue(labels map[string]string, o Options) error {
	...
	if field.IsContainer {
		...
		if iface, ok := field.Interface.(Labeler); ok {
			...
			return NewFieldError(field.Name, ErrSettingLabels)
		}
